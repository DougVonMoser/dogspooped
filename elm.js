...
var author$project$Main$closeAndUpdateFunc = F2(
	function (bigmodel, model) {
		var _n0 = model.timeAdjust;
		if (_n0.$ === 'InProgress') {
			var progressRecord = _n0.a;
			var _n1 = A2(author$project$Main$parseInputToPosix, progressRecord.inputValue, model.zone);
			if (_n1.$ === 'Just') {
				var updatedPosix = _n1.a;
...
var author$project$Main$ignoreNonDigits = function (raw) {
	var maybeChar = elm$core$List$head(
		elm$core$List$reverse(
			elm$core$String$toList(raw)));
	if (maybeChar.$ === 'Just') {
		var charry = maybeChar.a;
		return elm$core$Char$isDigit(charry) ? elm$core$Maybe$Just(raw) : elm$core$Maybe$Nothing;
	} else {
...
var author$project$Main$replaceColon = function (existing) {
	var _n0 = elm$core$String$length(existing);
	switch (_n0) {
		case 1:
			return ': ' + existing;
		case 2:
			return ':' + existing;
		case 3:
			return A2(elm$core$String$left, 1, existing) + (':' + A2(elm$core$String$right, 2, existing));
		case 4:
			return A2(elm$core$String$left, 2, existing) + (':' + A2(elm$core$String$right, 2, existing));
		default:
			return ':  ';
	}
};
...
var author$project$Main$groomInput = F2(
	function (existingInput, userAddedInput) {
		var pipeline = A2(
			elm$core$Maybe$map,
			author$project$Main$replaceColon,
			A2(
				elm$core$Maybe$andThen,
				author$project$Main$restrictFiveDigits,
				author$project$Main$ignoreNonDigits(
					A3(
						elm$core$String$replace,
						' ',
						'',
						A3(elm$core$String$replace, ':', '', userAddedInput)))));
		var deletemeyay = A2(elm$core$Debug$log, 'event?', userAddedInput);
		if (pipeline.$ === 'Just') {
			var updatedInput = pipeline.a;
			...
			switch (msg.$) {
				...
				case 'AdjustmentEvent':
					...
					if (_n5.$ === 'InProgress') {
						...
						var wasEnter = key === 13;
						var something = A2(elm$core$Debug$log, 'hi', key);
						var placeholder = _Utils_ap(
							progressRecord.inputValue,
							elm$core$String$fromChar(
								elm$core$Char$fromCode(key)));
						if (wasEnter) {
							return A2(author$project$Main$closeAndUpdateFunc, bigmodel, model);
						} else {
							var _n6 = A2(author$project$Main$groomInput, progressRecord.inputValue, placeholder);
							if (_n6.$ === 'KeepEditing') {
										...
										[
											...
											_List_fromArray(
												[
													...
													A2(rtfeldman$elm_css$Css$property, 'text-align', 'right'),
											...
											rtfeldman$elm_css$Html$Styled$Events$on,
											'keypress',
											A2(elm$json$Json$Decode$map, author$project$Main$AdjustmentEvent, rtfeldman$elm_css$Html$Styled$Events$keyCode))
										]),
									_List_Nil),
									A2(
									rtfeldman$elm_css$Html$Styled$button,
